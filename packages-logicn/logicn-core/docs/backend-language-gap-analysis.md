...
This document compares LogicN against common backend languages and identifies what
those languages support that LogicN does not currently define as a native language
feature.
...
decide what LogicN should support as a clear, safe backend language for:
...
LogicN should be designed around a small set of primary language rules. These rules
should guide future syntax, type-system, compiler, target and documentation
decisions.

```text
1. LogicN must stay developer friendly.
2. LogicN must have a clear learning curve.
3. LogicN must be a strict backend language.
4. LogicN must be AI-readable by design.
5. LogicN must be three-way logic aware.
6. LogicN must be security-first.
7. LogicN must be optimised for low memory usage.
8. LogicN must reduce unnecessary compute.
9. LogicN must be vector-ready.
10. LogicN must support repetitive AI compute tasks safely.
11. LogicN must be multi-state logic compatible, including Tri LogicN.
12. LogicN must be photonic and wavelength planning ready.
13. LogicN must remain CPU-compatible by default.
14. LogicN must be safe through explicit types, effects, permissions, reports and fallback.
15. LogicN must reduce ransomware-style attack risk through explicit file, network, package and permission controls.
16. LogicN is not a full framework.
17. LogicN may define contracts that an optional Secure App Kernel enforces.
```

These rules define what LogicN should become:
...
LogicN should be strict, but it should not be hard to read.
...
Strictness should help developers, not punish them. When LogicN rejects code, the
...
LogicN should be suitable for backend systems such as APIs, services, workers,
...
LogicN may still define safe application contracts that a separate Secure App
...
LogicN should be easy for AI coding tools to inspect without guessing.
...
LogicN should treat uncertainty as a first-class language concern.
...
LogicN should never silently convert:

```text
Tri to Bool
Decision to Bool
LogicN to a smaller logic width
unknown to allow
review to allow
```
...
Security should be part of normal LogicN syntax, not an afterthought.

LogicN should make these visible:
...
Ransomware-style attacks cannot be prevented by language design alone, but LogicN
should reduce the risk by making dangerous behaviour explicit and auditable.

LogicN should require clear permission boundaries for:
...
LogicN should make memory behaviour visible and predictable.
...
LogicN should reduce unnecessary computation before asking for more hardware.
...
Vector readiness should not make normal backend code harder to read. LogicN should
remain scalar-first and vector-aware.

### 8. Repetitive AI Compute Tasks

LogicN should support repetitive AI compute tasks as language-visible compute
...
LogicN should provide safe primitives that AI packages and compute backends can use.
...
Accelerator results must return to strict LogicN values.
Fallback must be explicit and reported.
CPU reference verification should be available where practical.
```

Normal LogicN code should stay readable even when the compiler can plan a pure
compute block for vector, GPU, photonic or wavelength execution.

---

## Current LogicN Position

The existing docs define LogicN as:
...
future-compatible with Tri, LogicN, photonic planning and wavelength planning
...
```text
LogicN should be a programming language first.
...
```text
LogicN core defines safety.
LogicN Secure App Kernel enforces safe runtime boundaries.
...
| Category | Meaning |
|---|---|
| Core LogicN candidate | Belongs in the language because it affects safety, typing, logic, memory, effects, compilation or compatibility |
| Standard library or package candidate | Useful for backend work, but should not become native syntax |
| Avoid as native LogicN | Conflicts with strictness, security, portability, AI readability or long-term stability |

---

## What LogicN Should Add for a Backend Language

These are language-level additions or clarifications that would make LogicN stronger
without turning it into a framework.

| Area | Why LogicN Needs It | Recommended LogicN Position |
|---|---|---|
...
| Logic width conversion | LogicN already has `Bool`, `Tri` and possible `LogicN` | Require explicit conversion policies and report lossy conversions |
| Photonic compute block rules | LogicN docs describe future photonic/wavelength planning | Keep photonic code as pure compute only, with CPU reference verification |
...
## What LogicN Should Not Add Natively

These features exist in some listed languages, but they would make LogicN less safe,
less portable or harder for AI tools to understand.

| Feature | Found In | LogicN Recommendation |
|---|---|---|
...
LogicN does not currently support unrestricted dynamic typing, metaclasses, monkey
...
LogicN does not currently define data frames, formula syntax or native statistics
...
LogicN should keep statistics, modelling and plotting in packages.
...
LogicN already aligns with Rust on memory safety, explicit errors and exhaustive
...
LogicN does not currently define multiple dispatch, a JIT model or native scientific
language syntax.
...
LogicN should avoid unrestricted multiple dispatch if it makes call resolution hard
...
LogicN does not currently define a VM bytecode, class-loader model, annotation
...
LogicN should avoid Java-style hidden framework magic through unrestricted runtime
...
LogicN does not support raw pointer programming, unrestricted templates, operator
...
LogicN should avoid making low-level hardware access a normal backend-language
feature.
...
LogicN does not currently define a stable ABI, C header generation, manual memory
...
LogicN should avoid C-style unchecked pointer arithmetic in normal code.
...
LogicN does not currently define properties, LINQ-style query syntax, attributes or
...
LogicN does not currently define higher-kinded types, implicit resolution or full
...
LogicN should avoid heavy implicit resolution because it can make code harder for
...
LogicN does not currently define matrix-first semantics, complex numbers or plotting
...
LogicN does not support prototype mutation, loose coercion, truthy/falsy control
...
LogicN should avoid JavaScript's implicit conversions.
...
LogicN does not currently define structural typing, conditional types or mapped
types.
...
LogicN should be careful with advanced type-level programming because it can become
...
LogicN does not currently define code-as-data or macros.
...
LogicN should avoid arbitrary source rewriting that hides behaviour from source maps
...
LogicN's `Tri` and `LogicN` are not the same as Prolog logic programming. LogicN does
...
LogicN should not make full Prolog-style backtracking a core backend feature unless
...
LogicN already aligns with optionals and explicit missing values, but does not
...
LogicN aligns with null safety and explicit state, but does not currently define
...
LogicN does not currently define goroutine-like concurrency, channels, defer or
...
LogicN does not support loose arrays, loose typing or web globals as native language
features.
...
LogicN should keep framework-style routing, templating, ORM and CMS features out of
the core language.
...
LogicN for future multi-state logic
...
Normal backend code should still read like normal LogicN:
...
LogicN should define backwards compatibility early because it wants to support
...
LogicN should stay understandable to AI tools by keeping syntax and semantics
predictable.
...
| 2 | Finalise `Bool`, `Tri`, `Decision` and `LogicN` conversion rules | Core to three-way and future photonic logic |
...
| 10 | Standardise diagnostics and AI report schemas | Keeps LogicN understandable for tools |
...
LogicN should not try to become Python, Rust, Java, C#, Go, Julia or TypeScript.

LogicN should take the safest language-level lessons:
...
LogicN's unique direction should remain:
