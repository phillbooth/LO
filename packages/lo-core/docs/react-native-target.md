# LO React Native Target Support

Status: Draft.

LO should support React Native in the same spirit as Dart/Flutter interop:

```text
LO is the language and compiler/toolchain.
React Native is an external mobile framework.
```

LO should generate safe JavaScript/TypeScript packages, schemas, bindings,
...
component syntax part of LO Core.
...
```text
LO can target React Native through generated packages and explicit interop.
...
Portable LO code should use LO types.

| LO type        | React Native / TypeScript boundary |
| -------------- | ---------------------------------- |
| `String`       | `string`                           |
| `Bool`         | `boolean`                          |
...
Normal LO code should not use JavaScript `undefined`, loose nullability,
...
This is generated adapter code. It is not LO syntax.

---

## Native Boundary Policy

React Native apps often cross into platform-specific native code. In LO, that
must be explicit.
...
React Native device features remain package/platform capabilities, not LO Core
APIs.
...
React Native UI component syntax in LO Core: unsupported for early LO
```
...
Do not make React Native a core LO framework.
```

LO should generate safe, typed, source-mapped and reportable code that React
Native apps can use.
