# LO Security Risk Feature Ranking

LO has two linked goals:
...
Make generated code traceable back to LO source.
```

No language can honestly promise perfect security, but LO can reduce preventable
...
| X    | Unsupported | Should not exist in normal LO                | Reject; migration or engine-lab discussion only |
...
Risk X must be rejected in normal LO code.
```
...
| Feature                         | Risk | Main risk                              | LO default                              | Required checks / failsafes                         |
| ------------------------------- | ---: | -------------------------------------- | --------------------------------------- | --------------------------------------------------- |
| Readable `.lo` syntax           | 0    | Ambiguous syntax can hide intent       | Core                                    | Stable grammar, formatter, parser diagnostics       |
...
| `Logic<N>`                      | 2    | Missing state handling                 | Core / advanced                        | Exhaustive `match`, no silent narrowing             |
...
| Feature / area                  | Risk | Main risk                                  | LO default                         | Required checks / failsafes                          |
| ------------------------------- | ---: | ------------------------------------------ | ---------------------------------- | ---------------------------------------------------- |
...
| Raw pointers                    | X    | Memory corruption                          | Unsupported in normal LO           | Engine-lab/native boundary only                      |
...
```text
LO-MEM-LEAK-001: Resource may escape its cleanup scope.

Resource:
  uploadStream

Source:
  src/uploads/process.lo:18:5
...
| Kernel/driver work              | X    | Privilege escalation and system damage     | Not normal LO                      | Late-stage maintainer-approved research only        |
...
| Shell execution                 | X    | Remote code execution                      | Unsupported in normal LO           | Explicit admin/tooling boundary only                |
| `eval`                          | X    | Remote code execution                      | Unsupported in normal LO           | Reject; engine-lab only                             |
...
LO should be easy for AI to understand without making security weaker.
...
LO should fail closed when a feature is risky and unclear.
...
If eval or shell execution appears in normal LO, reject.
```
...
```json
{
  ...
  "source": "boot.lo:22",
  "reason": "Native bindings can bypass LO memory safety.",
...
LO should be nice to write because safe code is the shortest path.

LO should be easy for AI to understand because every risky feature is explicit,
