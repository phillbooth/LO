...
} from "../../lo-devtools-project-graph/dist/index.js";
...
async function readGeneratedGraph(context: CliContext): Promise<ProjectGraph> {
  ...
  const graphPath =
    graphFlagValue === undefined
      ? join(resolveOutputDirectory(context), "lo-devtools-project-graph.json")
  ...
  try {
    ...
    throw new Error(
      "Project graph not found. Run `node packages-lo\\lo-core-cli\\dist\\index.js graph --out build\\graph` from the repository root first.",
    );
  }
}
...
function createGraphOutputPaths(
  ...
  return {
    ...
    jsonPath: join(outputDirectory, "lo-devtools-project-graph.json"),
    htmlPath: join(outputDirectory, "lo-devtools-project-graph.html"),
