...
`lo-core-compute` is the package for LO compute planning concepts.
...
`lo-core-compute` should describe how work can be planned across targets. It should
...
For AI inference, `lo-core-compute` may express preference and fallback order such
as:
...
For distributed tensor or AI workloads, `lo-core-compute` may express interconnect
preference:
...
belongs in `lo-ai-lowbit`, CPU capability planning belongs in `lo-target-cpu`,
...
```text
lo-core-compute plans work.
lo-ai describes AI inference.
lo-ai-lowbit describes low-bit AI backend inference.
