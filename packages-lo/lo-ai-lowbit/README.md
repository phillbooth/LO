...
`lo-ai-lowbit` is the package for low-bit AI inference contracts.
...
`lo-ai-lowbit` does not define LO `Tri` semantics. Low-bit and ternary AI
weights are model weights; LO `Tri` and `Logic<N>` belong in `lo-core-logic`.

`lo-ai-lowbit` should not own generic AI contracts, CPU feature detection or
...
```text
lo-ai-lowbit adapts low-bit AI inference.
bitnet is one backend inside lo-ai-lowbit.
lo-ai defines generic AI inference contracts.
lo-core-logic defines language-level ternary logic.
```
