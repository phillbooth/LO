# LO Storage-Aware Performance

Status: draft language, compiler, tooling and report contract.

LO should not claim direct support for M.2 SSDs, NVMe controllers or any
specific storage hardware. Operating systems, firmware, controllers and drivers
own physical storage access.

LO may be storage-aware. That means the compiler, runtime and tools can use
...
LO can perform better by understanding the project, storage, memory, schemas,
effects and target hardware before the program runs.
```

Compared with Python, LO can gain speed through native compilation, strict
typed execution, streaming-first data handling and fewer runtime checks.

Compared with C++, LO should not claim to be generally faster. C++ remains one
of the strongest raw-performance languages when written well. LO's opportunity
...
LO should report detected storage facts only when they are available and
...
LO should still treat fast storage as a performance opportunity, not a
...
LO should not cache by default:
...
If a cache cannot be used safely, LO should calculate the result without using
...
LO may use the project graph to avoid recompiling unaffected work:
...
LO should prefer streaming for large files:
...
LO may support memory-mapped read-only files where the platform supports it:
...
LO storage-aware performance should not:
