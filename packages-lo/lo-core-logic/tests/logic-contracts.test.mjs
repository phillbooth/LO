...
describe("lo-core-logic contracts", () => {
  ...
  it("rejects malformed logic definitions before use", () => {
    ...
    assert.equal(
      diagnostics.some(
        (diagnostic) => diagnostic.code === "LO_LOGIC_DUPLICATE_STATE",
      ),
      true,
    );
    assert.throws(
  ...
  it("reports duplicate, invalid and incomplete truth table rows", () => {
    ...
    assert.equal(
      diagnostics.some(
        (diagnostic) =>
          diagnostic.code === "LO_LOGIC_DUPLICATE_TRUTH_TABLE_ROW",
      ),
      true,
    );
    assert.equal(
      diagnostics.some(
        (diagnostic) => diagnostic.code === "LO_LOGIC_INVALID_INPUT_STATE",
      ),
      true,
    );
    assert.equal(
      diagnostics.some(
        (diagnostic) => diagnostic.code === "LO_LOGIC_INCOMPLETE_TRUTH_TABLE",
      ),
      true,
    );
  });
