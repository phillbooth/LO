...
export const DEFAULT_PROJECT_GRAPH_SCAN_POLICY: ProjectGraphScanPolicy = {
  include: ["README.md", "docs/**", "packages/**", "packages-lo/**"],
...
export function createWorkspaceProjectGraph(
  ...
  const graphPackage = packages.find((item) => item.name === "lo-devtools-project-graph");
...
export function createDefaultProjectGraphOutputManifest(
  ...
  return {
    jsonPath: `${outputDirectory}/lo-devtools-project-graph.json`,
    htmlPath: `${outputDirectory}/lo-devtools-project-graph.html`,
    reportPath: `${outputDirectory}/LO_GRAPH_REPORT.md`,
    aiMapPath: `${outputDirectory}/lo-ai-map.md`,
    generatedFiles: [
      `${outputDirectory}/lo-devtools-project-graph.json`,
      `${outputDirectory}/lo-devtools-project-graph.html`,
